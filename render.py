...
class ScreenProcessor:
    ...
    def init_text(self):
        ...
        self.text = {fontsize: pygame.font.Font(fontpath, fontsize) for fontsize in range(8, 100)}
...
def pipeline(queue, qout, config):
    import signal
    ...
    screen = pygame.display.set_mode(display_size, pygame.FULLSCREEN) # 创建screen
        ...
        while True:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    pos = pygame.mouse.get_pos()
                    x, y = processor.to_map(pos)
                    if 1460<x<1540 and  730 < y < 800 :
                        qout.put("throw")
